...
| Symptom | Cause | Fix |
|---|---|---|
| Every chap-core config POST is a `422`, but `chapkit test` works (chapkit < 2.1.0) | `BaseConfig` declared `prediction_periods: int` with **no default**, and chap-core never sends it | Fixed in chapkit 2.1.0 (`BaseConfig` defaults it to 3). On older chapkit, declare `prediction_periods: int = Field(default=3, description=...)` on your Config subclass. **[mstl] [multistep]** |
| Service answers 200, jobs succeed, numbers are the defaults - the requested tunables were ignored (chapkit < 2.1.0) | chap-core posts `{"name": ..., "user_option_values": {...}}`. `BaseConfig` has `extra="allow"`, so the nested dict was stored verbatim as an unknown extra, every declared field kept its default, and `dump_config_yaml(..., "chap_core")` then emitted `user_option_values: {user_option_values: {...}, n_samples: <default>}` | Fixed in chapkit 2.1.0: `BaseConfig` hoists `user_option_values` itself (CLIM-1122, dhis2-chap/chapkit#110). Pin `chapkit>=2.1.0` and do not add your own validator. On older chapkit, add the `@model_validator(mode="before")` from `config-and-runner-snippets.md`. Either way use `ShellModelRunner(config_format="chap_core")` for migrated scripts. This is the failure a smoke test will not catch and a parity test will. **[mstl]** |
...
| `run_info.prediction_length` has no effect (chapkit < 2.1.0) | Older chapkit dropped chap-core's request-level `run_info` | chapkit 2.1.0 resolves the horizon per request (run_info, else the future frame, else the stored config), hands it to the runner as `config.prediction_periods` / the `prediction_periods` key in `config.yml`, and checks it against `min`/`max_prediction_periods`. Models may keep deriving the horizon from the future rows. The train-time value depends on chap-core: master from 2026-09-18 (CLIM-1123, dhis2-chap/chap-core#614) sends the requested horizon clamped to the service's declared bounds; released chap-core 2.3.0 sends a placeholder of 1, so a model that trains for the horizon must state which chap-core it needs. **[mstl]** |
...
| Symptom | Cause | Fix |
|---|---|---|
| `chap eval` aborts with `ValidationError ... period_type Input should be 'weekly' or 'monthly' [input_value='any']` | Released chap-core (2.3.0; also the `ghcr.io/dhis2-chap/chap-core:latest` image once that tag tracks releases) validates `/api/v1/info` against a `PeriodType` enum without `any`, and its `$register` endpoint returns 422 for the same reason. chap-core `master` (the `:master` image, 2.4.0.dev0 and later) accepts it | Not a defect in your service. Check which chap-core the deployment runs. Verify the rest of the path by temporarily setting `PeriodType.monthly`, run `chap eval`, then revert, or test against `:master`. Say so in the PR. **[mstl]** |
| `chap eval` never gets past config creation (chapkit < 2.1.0) | The chap-core-shaped config body is not handled | This is the `user_option_values` hoist again; fixed in chapkit 2.1.0. `chap eval --run-config.is-chapkit-model` is the cheapest end-to-end check of the real chap-core path. **[mstl]** |
| `uv lock` says only an older chapkit exists minutes after a release | uv's package index cache is stale | `uv lock --refresh-package chapkit`. **[mstl]** |
| `chapkit test`, `$validate` or a chap-core job rejects the horizon with `prediction_periods_out_of_bounds` (chapkit >= 2.1.0) | The bounds check now uses the real horizon of the request instead of a config constant | Declare honest `min_prediction_periods` / `max_prediction_periods`; `chapkit test` and `$generate-sample-data` clamp their generated horizon to those bounds, and chap-core's CLI eval path wraps the model in `ExtendedPredictor` above the max. chap-core's REST worker prediction and backtest paths do not (open follow-up), so a UI request beyond the declared max fails the predict job at the service. **[docs]** |
